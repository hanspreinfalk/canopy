...
final class AssemblyAIStreamingTranscriptionProvider: CustomTranscriptionProvider {
    ...
    private static let tokenProxyURL = "https://oceanic-opossum-563.convex.site/transcribe-token"
    ...
    ) async throws -> any CustomStreamingTranscriptionSession {
...
private final class AssemblyAIStreamingTranscriptionSession: NSObject, CustomStreamingTranscriptionSession {
    ...
    private let stateQueue = DispatchQueue(label: "com.hanspreinfalk.assemblyai.state")
    private let sendQueue = DispatchQueue(label: "com.hanspreinfalk.assemblyai.send")
    private let audioPCM16Converter = CustomPCM16AudioConverter(targetSampleRate: targetSampleRate)
